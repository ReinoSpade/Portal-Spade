...
const PAGE_THEME={home:"home",guia:"guide",grimorio:"grimoire",biblioteca:"grimoire",jornal:"journal",comunicados:"journal",cards:"battle",simulador:"battle",emblemas:"home",missoes:"battle",eventos:"battle",cronograma:"battle",ranking:"battle",casas:"home",jogadores:"home",status:"home",cargos:"home",hierarquia:"home",admin:"home","admin-login":"home"};
  ...
  if(f){
    f.innerHTML=featured
      ? `<div class="announcement-feature">
          <div class="announcement-feature-sigil">♠</div>
          ...
          <span class="announcement-category">${escapeHtml(featured.category||"INFORMATIVO")}</span>
          <h2>${escapeHtml(featured.title)}</h2>
          <p>${escapeHtml(featured.body||"")}</p>
          <div class="announcement-signature"><span>ADMINISTRAÇÃO DO REINO</span><span>SPADE • OFICIAL</span></div>
        </div>`
      : "";
  }
...
function renderMissionsPublic(items){
  ...
  grid.innerHTML=arr.length?arr.map(m=>{const rewardParts=[];if(m.reward_yuls)rewardParts.push(`<span>🪙 ${money(m.reward_yuls)} Yuls</span>`);if(m.reward_exp)rewardParts.push(`<span>✨ ${money(m.reward_exp)} EXP</span>`);if(m.reward_cards)rewardParts.push(`<span>🃏 ${escapeHtml(m.reward_cards)}</span>`);return `<article class="mission-card ${m.status==="EM_ANDAMENTO"?"active":""}" id="mission-${m.id}"><div class="mission-card-banner"><span class="mission-status-badge ${String(m.status||'').toLowerCase()}">${escapeHtml(m.status)}</span><span class="mission-type-mark">⚔</span></div><div class="mission-card-top"><b>${escapeHtml(missionLabel(m))}</b><small>#${Number(m.id)}</small></div><p>${escapeHtml(m.description||"Sem descrição publicada.")}</p><div class="mission-meta"><span>📅 ${escapeHtml(missionDate(m.start_at))}</span><span>⏳ ${escapeHtml(missionDate(m.end_at))}</span></div><div class="mission-instructions"><b>Instruções</b><p>${escapeHtml(m.instructions||"Consulte as instruções oficiais no Portal.")}</p></div><div class="mission-rewards"><small>RECOMPENSAS</small><div>${rewardParts.join('')||'<span>Sem recompensa cadastrada</span>'}</div></div></article>`}).join(""):`<div class="panel"><h3>Nenhuma missão encontrada.</h3><p>Ajuste os filtros ou aguarde uma nova atividade oficial.</p></div>`;
...
function renderJournal(editions,news,o){
  ...
  if(editionEl){
    editionEl.innerHTML=editions?.length?editions.map(e=>`<article class="edition"><div class="edition-cover ${e.cover_url?"has-image":""}" ${e.cover_url?`style="background-image:url('${escapeHtml(e.cover_url)}')"`:""}>${e.cover_url?`<span class="edition-cover-overlay"></span>`:`<span>♠</span><small>${escapeHtml(e.edition||"EDIÇÃO")}</small><b>SPADE</b><em>${escapeHtml(String(e.date||""))}</em>`}<div class="edition-cover-stamp">${escapeHtml(e.edition||"EDIÇÃO")}</div></div><div class="edition-card-copy"><div class="edition-card-meta-row"><span>${escapeHtml(String(e.date||""))}</span><span>${Number(e.article_count||0)} matérias</span></div><h3>${escapeHtml(e.title)}</h3><p>${escapeHtml(e.description||"")}</p><div class="actions"><button class="gold small journal-edition-open" type="button" data-open-edition="${e.id}">Ler edição</button>${e.pdf_url?`<a class="outline small" href="${escapeHtml(e.pdf_url)}" target="_blank" rel="noopener">PDF</a>`:""}</div></div></article>`).join(""):`<div class="panel"><h3>Nenhuma edição publicada.</h3></div>`;
...
function emblemStageCommercialDefault(n){return ({1:'Recruta',2:'Combatente',3:'Elite',4:'Lendário'})[n]||emblemStageLabel(n);}
...
function renderEmblemCard(e,{compact=false}={}){
  const locked=!e.unlocked; const stage=e.current_stage||0; const stages=Array.isArray(e.stages)?e.stages:[];
  const progress=e.progress?.percent??(stage&&stages.length&&stage>=stages[stages.length-1].stage_number?100:0);
  const track=stages.map(st=>{const visual=e.stage_mode==='ADVANCED_SUPREME'?(st.stage_number===3?'A':'S'):String(st.stage_number);return `<span class="${stage>=st.stage_number?'unlocked':''}" title="${escapeHtml(emblemStageLabel(st.stage_number))}${st.name?` · ${escapeHtml(st.name)}`:''}">${visual}</span>`;}).join('');
  const commercial=e.current_stage_name?` · ${escapeHtml(e.current_stage_name)}`:'';
  const nextText=e.next_stage?`${escapeHtml(e.next_stage.stage_label||emblemStageLabel(e.next_stage.stage_number))}${e.next_stage.name?` · ${escapeHtml(e.next_stage.name)}`:''}`:'';
  return `<article class="emblem-card ${locked?'locked':''} ${e.featured?'featured':''}"><div class="emblem-card-art"><span>${locked?'❔':escapeHtml(e.icon||'🏅')}</span></div><div class="emblem-card-body"><div class="emblem-card-top"><span class="tag">${escapeHtml(e.category||'Especial')}</span><small>${escapeHtml(e.rarity_label||e.rarity||'')}</small></div><h3>${escapeHtml(e.name)}</h3><p>${escapeHtml(e.description||'')}</p><div class="emblem-stage-track ${e.stage_mode==='ADVANCED_SUPREME'?'special':''}">${track}</div><div class="emblem-stage-label">${stage?`🏅 ${escapeHtml(e.stage_label||emblemStageLabel(stage))}${commercial}`:'🔒 Bloqueado'}</div>${e.next_stage?`<div class="emblem-progress"><div><small>Próximo: ${nextText}</small><b>${progress}%</b></div><div class="emblem-progress-bar"><i style="width:${progress}%"></i></div></div>`:''}${e.featured?'<span class="emblem-featured-badge">⭐ Em destaque</span>':''}${stage&&!compact?`<button class="outline dark-outline small" type="button" data-emblem-feature="${e.id}" data-featured="${e.featured?'1':'0'}">${e.featured?'☆ Remover destaque':'⭐ Destacar'}</button>`:''}</div></article>`;
}
...
function renderEmblemsPage(){const root=qs('#emblemsRoot');if(!root)return;const items=state.emblems||[],unlocked=items.filter(x=>x.unlocked),featured=items.filter(x=>x.featured),special=items.filter(x=>x.stage_mode==='ADVANCED_SUPREME');const percent=items.length?Math.round((unlocked.length/items.length)*100):0;root.innerHTML=`<section class="emblem-hall-hero"><div class="emblem-hall-mark">🏅</div><div><p class="eyebrow">HALL DOS EMBLEMS</p><h2>Marcas que contam sua história.</h2><p>Colecione, evolua e destaque os símbolos que representam sua trajetória em Spade.</p></div><div class="emblem-hall-meter"><span>PROGRESSO DA COLEÇÃO</span><b>${percent}%</b><i><em style="width:${percent}%"></em></i></div></section><div class="emblem-summary panel"><div><p class="eyebrow">MINHA COLEÇÃO</p><h2>${unlocked.length} / ${items.length} Emblems</h2><p>Progresso, raridade, evolução e vitrine em um único lugar.</p></div><div class="emblem-summary-stats"><span><small>CONQUISTADOS</small><b>${unlocked.length}</b></span><span><small>EM DESTAQUE</small><b>${featured.length} / 5</b></span><span><small>ESPECIAIS</small><b>${special.length}</b></span></div></div><div class="emblem-collection-head"><div><p class="eyebrow">COLEÇÃO</p><h2>Todos os Emblems</h2></div><span class="emblem-collection-note">Evolua seus Emblems conforme os requisitos do Reino.</span></div><div class="emblem-grid">${items.map(e=>renderEmblemCard(e)).join('')||'<div class="panel">Nenhum Emblem foi cadastrado ainda.</div>'}</div>`;qsa('[data-emblem-feature]').forEach(b=>b.onclick=async()=>{try{await api(`/api/me/emblems/${b.dataset.emblemFeature}/feature`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({featured:b.dataset.featured!=='1'})});await loadEmblemsPage();}catch(e){alert(e.message)}});}
async function loadAdminEmblems(){const list=qs('#adminEmblemList');if(!list)return;try{const d=await adminApi('/api/admin/emblems');state.adminEmblems=d.emblems||[];list.innerHTML=state.adminEmblems.map(e=>`<div class="emblem-admin-row"><div class="emblem-admin-row-main"><span class="emblem-admin-icon">${escapeHtml(e.icon||'🏅')}</span><div><b>${escapeHtml(e.name)}</b><small>${escapeHtml(e.category)} • ${escapeHtml(e.rarity)} • ${e.stage_mode==='ADVANCED_SUPREME'?'Avançado + Supremo':'4 níveis'} • ${e.active?'ATIVO':'INATIVO'} • ${e.owners} possuidor(es)}</small><p>${escapeHtml(e.description||'')}</p></div></div><div class="emblem-admin-actions"><button type="button" data-emblem-edit="${e.id}">✎</button><button type="button" data-emblem-grant="${e.id}">🎁</button><button type="button" data-emblem-revoke="${e.id}">↩</button><button type="button" class="delete" data-emblem-delete="${e.id}">×</button></div></div>`).join('')||'<div class="admin-history-empty">Nenhum Emblem cadastrado.</div>';qsa('[data-emblem-edit]').forEach(b=>b.onclick=()=>setAdminEmblemForm(state.adminEmblems.find(e=>Number(e.id)===Number(b.dataset.emblemEdit))));qsa('[data-emblem-delete]').forEach(b=>b.onclick=async()=>{if(!confirm('Desativar este Emblem?'))return;try{await adminApi(`/api/admin/emblems/${b.dataset.emblemDelete}`,{method:'DELETE'});await loadAdminEmblems();}catch(e){alert(e.message)}});qsa('[data-emblem-grant]').forEach(b=>b.onclick=()=>grantAdminEmblem(Number(b.dataset.emblemGrant)));qsa('[data-emblem-revoke]').forEach(b=>b.onclick=()=>revokeAdminEmblem(Number(b.dataset.emblemRevoke)));}catch(e){list.innerHTML=`<div class="admin-history-empty">${escapeHtml(e.message)}</div>`;}}
function setAdminEmblemForm(e=null){qs('#adminEmblemId').value=e?.id||'';qs('#adminEmblemName').value=e?.name||'';qs('#adminEmblemIcon').value=e?.icon||'🏅';qs('#adminEmblemCategory').value=e?.category||'Especial';qs('#adminEmblemRarity').value=e?.rarity||'COMUM';qs('#adminEmblemOrigin').value=e?.origin||'Reino Spade';qs('#adminEmblemDescription').value=e?.description||'';qs('#adminEmblemStageMode').value=e?.stage_mode||'FOUR';const st=e?.stages||[];for(let i=1;i<=4;i++){const x=st.find(v=>Number(v.stage_number)===i)||{};qs(`#emblemStage${i}Name`).value=x.name||emblemStageCommercialDefault(i);qs(`#emblemStage${i}Description`).value=x.description||'';qs(`#emblemStage${i}Req`).value=x.requirements?JSON.stringify(x.requirements,null,2):'';}toggleAdminEmblemStageMode();qs('#adminEmblemSecret').checked=!!e?.secret;qs('#adminEmblemAuto').checked=e?!!e.auto_award:true;qs('#adminEmblemActive').checked=e?!!e.active:true;qs('#adminEmblemSaveBtn').textContent=e?'Salvar Emblem':'Criar Emblem';qs('#adminEmblemError').textContent=e?'Editando Emblem.':'';}
function clearAdminEmblemForm(){setAdminEmblemForm(null);}
function toggleAdminEmblemStageMode(){const mode=qs('#adminEmblemStageMode')?.value||'FOUR';qsa('[data-emblem-stage]').forEach(el=>{const n=Number(el.dataset.emblemStage);el.style.display=(mode==='ADVANCED_SUPREME'&&n<3)?'none':'';});const sel=qs('#adminEmblemGrantStage');if(sel){Array.from(sel.options).forEach(o=>{o.hidden=mode==='ADVANCED_SUPREME'&&Number(o.value)<3;});if(mode==='ADVANCED_SUPREME'&&Number(sel.value)<3)sel.value='3';}}
function collectAdminEmblemForm(){const mode=qs('#adminEmblemStageMode').value;const allowed=mode==='ADVANCED_SUPREME'?[3,4]:[1,2,3,4];const stages=[];for(const i of allowed){const raw=qs(`#emblemStage${i}Req`).value.trim();let requirements={logic:'ALL',conditions:[]};if(raw){try{requirements=JSON.parse(raw);}catch{throw new Error(`Requisitos do estágio ${i} estão com JSON inválido.`)}}stages.push({stage_number:i,name:qs(`#emblemStage${i}Name`).value,description:qs(`#emblemStage${i}Description`).value,requirements});}return {name:qs('#adminEmblemName').value,icon:qs('#adminEmblemIcon').value,category:qs('#adminEmblemCategory').value,rarity:qs('#adminEmblemRarity').value,origin:qs('#adminEmblemOrigin').value,description:qs('#adminEmblemDescription').value,stage_mode:mode,secret:qs('#adminEmblemSecret').checked,auto_award:qs('#adminEmblemAuto').checked,active:qs('#adminEmblemActive').checked,stages};}
...
async function revokeAdminEmblem(id){const playerId=Number(prompt('ID do jogador que perderá este Emblem:')||0);if(!playerId)return;if(!confirm('Revogar este Emblem deste jogador?'))return;try{await adminApi(`/api/admin/emblems/${id}/grant/${playerId}`,{method:'DELETE'});await loadAdminEmblems();alert('Emblem revogado.');}catch(e){alert(e.message)}}
...
function updateContextNav(){
  ...
  qsa(".player-only-nav").forEach(el=>{
    const id=el.id;
    const visible=id==="cardsNav"?canSeeCards:id==="notificationsNav"?canSeeNotifications:id==="grimoireNav"?canSeeGrimoire:id==="emblemsNav"?canSeeEmblems:logged;
...
function cardElementSlug(v){return String(v||"neutro").normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/^-|-$/g,"")||"neutro";}
...
qs("#adminEmblemSaveBtn")?.addEventListener("click",saveAdminEmblem);
...
qs("#adminEmblemStageMode")?.addEventListener("change",toggleAdminEmblemStageMode);
